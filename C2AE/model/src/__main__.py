...
from model.src.ml_metrics import all_metrics
...
class Model(object):
    ...
    def run_epoch(self, sess, data, summarizer, epoch, fold=0):
        err = list()
        i = 0
        step = epoch
        merged_summary = self.summarizer.merge_all()
        for X, Y, tot in self.data.next_batch(data, fold):
    ...
    def run_eval(self, sess, data, summary_writer=None, step=0, fold=0):
        y, y_pred, loss_, metrics, p_k = list(), list(), 0.0, None, None
        accuracy, loss = 0.0, 0.0
        merged_summary = self.summarizer.merge_all()
        i = 0
        for X, Y, tot in self.data.next_batch(data, fold=fold):
    ...
    def fit(self, sess, summarizer, fold):
        '''
         - Patience Method : 
        ...
        while self.epoch_count < max_epochs :
            ...
            average_loss, tr_step = self.run_epoch(sess, "train", summarizer['train'], self.epoch_count, fold=fold)
            duration = time.time() - start_time
            if not self.config.debug :
                if self.epoch_count % self.config.epoch_freq == 0 :
                    val_loss, _, _, _ = self.run_eval(sess, "validation", summarizer['val'], tr_step)
                    test_loss, _, metrics, patk= self.run_eval(sess, "test", summarizer['test'], tr_step, fold=fold)
...
def train_model(config, fold):
    ...
    with sess:
        ...
        loss_dict = model.fit(sess, summary_writers, fold)
        return loss_dict

def main():
    args = Parser().get_parser().parse_args()
    args.dataset = 'emotions'
    config = Config(args)
    n_fold = 5
    rets = np.zeros((5, 11))

    for fold in range(n_fold):
        loss_dict = train_model(config, fold)
        metrics = loss_dict['test_metrics']
        if config.debug == False:
            output = "=> Best Train Loss : {}, Test Loss : {}, Test Accuracy : {}"\
                .format(loss_dict["train"], loss_dict["test_loss"], loss_dict["test_accuracy"])
        else:
        ...
        output += "\n=> Test : Coverage = {}, Average Precision = {}, Micro Precision = {}, Micro Recall = {}, Micro F Score = {}".format(
            metrics['coverage'], metrics['avg_precision'], metrics['micro_precision'], metrics['micro_recall'],
            metrics['micro_f1'])
        output += "\n=> Test : Macro Precision = {}, Macro Recall = {}, Macro F Score = {}"\
        ...
        for i, key in enumerate(metrics):
            rets[fold][i] = metrics[key]

    means = np.mean(rets, axis=0)
    stds = np.std(rets, axis=0)

    save_name = f'../results/{args.dataset}'
    with open(save_name, "w") as f:
        for i, key in enumerate(metrics):
            print("{metric}\t{means:.4f}±{std:.4f}".format(metric=key, means=means[i], std=stds[i]))
            f.write("{metric}\t{means:.4f}±{std:.4f}".format(metric=key, means=means[i], std=stds[i]))
            f.write("\n")
